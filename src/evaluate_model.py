...
import wandb
wandb.init(project="seoul-bikes-sharing")
...
def get_metrics(predictions, X_test_data, y_test_data, metrics_path):
    ...
    wandb.log({"MAE": mae, "RMSE": rmse, "R^2": r2_score})
...
if __name__ == "__main__":
    ...
    path = Path().cwd()
    model_path = path.joinpath('models', 'bikes_model.pkl')
    metrics_path = path.joinpath("reports", 'metrics.json')
    test_path = path.joinpath("data", 'processed', 'test.parquet')
    
    X_test = pd.read_parquet(test_path).drop("date", axis=1)
